...
void initialize(int * data, int n){
  ...
  for(page_num = 0; page_num < NUM_FRAMES; page_num++){
    ...
    if(page_num < num_pages_n){
      ...
      entry_ptr->valid = 1;
      // increment counter of accesses
      timer += 1;
      entry_ptr->lru_counter = timer;
...
// takes a virtual address returns physical address
// -1 if doesn't exist
physical_addr translate_addr(virtual_addr va){
  // TODO #1: implement this function
  int offset = va % PAGE_SIZE;
  int index = va / PAGE_SIZE;


  // now we want to look at the frame at index to see if it exists
  // page exists in physical mem
  if (page_table[index].valid == 1) {
    // increment counter of accesses
    timer += 1;
    page_table[index].lru_counter = timer;
    int frameNum = page_table[index].frame_number;
    // doing the opposite of what we did to find the index & offset from va
    physical_addr pa = (frameNum * PAGE_SIZE) + offset;
    return pa;
  } else {
    return -1;
  }
...
void handle_page_fault(virtual_addr va){
  ...
  if(strategy == RANDOM){
    ...
    for (int i = 0; i < NUM_PAGES; i++) {
      // save the lru_counter
      long counter = page_table[i].lru_counter;

      // if the counter is less than the min
      if (page_table[i].valid == 1) {
        if (counter < min) {

          // set the min and update the page to evict
          min = counter;
          evicted_page = i;
        }
      }
    ...
    // set pointer to the page number index we want to evict
    evicted_entry_ptr = page_table + evicted_page;
